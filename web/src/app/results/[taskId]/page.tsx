...
export default function ResultsPage() {
  ...
  const isSingleScrape = data.task_scrape_type === "single";

  const language = isSingleScrape
    ? data.single_result.format === "json"
      ? "json"
      : "csv"
    : null;

  const filename = isSingleScrape
    ? language === "json"
      ? "data.json"
      : "data.csv"
    : null;

  const code = isSingleScrape
    ? (data.single_result.data_json ?? data.single_result.data_csv)
    : null;
      ...
      <div className="container mx-auto pt-28 pb-20">
        ...
        <Card className="border-white/10">
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                <p className="text-md text-white/70">Task ID</p>
                <p className="text-white">{data.task_id}</p>
              </div>
              <div>
                <p className="text-md text-white/70">Status</p>
                <p className="text-white capitalize">{data.task_status}</p>
              </div>
              <div>
                <p className="text-md text-white/70">Scrape Type</p>
        ...
        {isSingleScrape ? (
          <Card className="mt-6 border-white/10">
            ...
            <CardContent>
              <Tabs defaultValue="json" className="w-full">
                <TabsContent value={language ?? "json"}>
                  <CodeBlock
          ...
          <div className="mt-6 text-white/70">
            <p>
              You can access the data via your own AI Agent by connecting to out
              MCP server! All you need is your taskId:
            </p>
